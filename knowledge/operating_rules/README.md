---
relations:
  role: doc
  domain: knowledge
  upstream: []
  downstream: []
---
# Defines rules for operational procedures.

## Where to write / Where not to write
*   **Write here:** Rules, guidelines, and instructions for system operation, maintenance, and incident response.
*   **Do NOT write here:** Agent-specific policies (use `agents/policies/`), canonical governance policies (use `governance/`).

## Key entrypoints
*   None explicitly defined, but related to general system operation.
